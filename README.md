# Computer-Graphics
## Модуль 1
### 1.1.1 Создать программу, которая рисует отрезок между двумя точками, заданными пользователем
...
### 1.1.2 Создать программу, которая рисует окружность с заданным пользователем радиусом
...
### 1.2.1 Реализация алгоритма Сезерленда-Коэна
...
### 1.2.2 Реализация алгоритма Цирруса-Бека
...
```
## 1.3 Алгоритмы заполнения

### 1.3.1 Алгоритм заполнения замкнутых областей посредством "затравки"
...
### 1.3.2 Алгоритм заполнения замкнутых областей посредством горизонтального сканирования
...
print(f'GIF сохранен в файл: {gif_filename}')
```
### 2.2.1 Вращаем без прозрачности (виден только перед фигуры)
```python
import numpy as np
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D
...
# Параметры GIF
gif_filename = 'kruchu v 240 fps2.gif'
frames = []
num_frames = 240  # Количество кадров для 1 секунды анимации
...
print(f'GIF сохранен в файл: {gif_filename}')
```
### 2.3.1 Вращаем фигуру с источником света и затемнением (освещением) граней согласно положению источника
```python
import numpy as np
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D
...
# Параметры GIF
gif_filename = 'kruchu_v_240_fps.gif'
frames = []
num_frames = 240  # Количество кадров для 1 секунды анимации
